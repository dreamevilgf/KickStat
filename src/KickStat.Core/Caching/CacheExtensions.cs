...
{
    public static T? Get<T>(this ICache cacheManager, string key, Func<T> acquire) => Get(cacheManager, key, TimeSpan.FromSeconds(60), acquire);

    /// <summary>
    /// Взять элемент из кэша и если его нет в кэше, выполнить функцию по его получению и положить в кэш
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="cacheManager">Объект кэша</param>
    ...
    /// <returns></returns>
    public static T? Get<T>(this ICache cacheManager, string key, TimeSpan cacheTime, Func<T?> acquire)
    {
    ...
    public static async Task<T?> Get<T>(this ICache cacheManager, string key, Func<Task<T>> acquire) => await Get(cacheManager, key, TimeSpan.FromSeconds(60), acquire);
    ...
    /// <param name="acquire">Делегат для получения элемента</param>
    /// <returns></returns>
    public static async Task<T?> Get<T>(this ICache cacheManager, string key, TimeSpan cacheTime, Func<Task<T>> acquire)
    {
        if (cacheManager.IsSet(key))
            return cacheManager.Get<T>(key);
    ...
    /// <returns></returns>
    public static T? GetWithExclusiveLock<T>(this ICache cache, string key, TimeSpan cacheTime, Func<T> acquire)
    {
        if (cache.IsSet(key))
    ...
    /// <param name="acquire"></param>
    /// <returns></returns>
    public static async Task<T?> GetWithExclusiveLock<T>(this ICache cache, string key, TimeSpan cacheTime, Func<Task<T>> acquire)
    {
        if (cache.IsSet(key))
            return cache.Get<T>(key);
